...
def plot_traj_distribution_1_joint(save_path, config, dof, mean_traj_1, traj_std_1, mean_traj_2=np.zeros(shape=(100,)),
            ...
            # Degree of Freedom task. Change colors if you like.
            x = np.linspace(0, 100, 100)
            plt.plot(q1true, 'y', label='True', linewidth=0.5)   # plot true mean
            plt.legend(loc=1, fontsize='x-small')
            plt.plot(q1righttrue, 'g', linewidth=0.5)   # plot true std, 99%
            plt.plot(q1lefttrue, 'g', linewidth=0.5)
            plt.plot(q1righttrue_1, 'g', linewidth=0.5)     # plot true std, 66%
            plt.plot(q1lefttrue_1, 'g', linewidth=0.5)
            plt.fill_between(x, q1righttrue.reshape(100, ), q1lefttrue.reshape(100, ), alpha=0.25, facecolor='green')
            plt.fill_between(x, q1righttrue_1.reshape(100, ), q1lefttrue_1.reshape(100, ), alpha=0.25, facecolor='green')
