...
def load_images(ids, img_path, resize_shape=(256, 256, 3)):
    """
...
def load_encoded_image(img_path, encoder, resize_shape=(256, 256, 3)):
    """
...
def load_encoded_images(ids, img_dir, encoder, resize_shape=(256, 256, 3)):
    """
...
def get_id(filename):
    """
    Gets  the id of a .png file     # .json
    """
...
def get_ids(data_dir):
    """
    Gets a list of ids of the .png files    # .json
    """
...
def load_weights_mean(json_path, mean_key="mean_vector"):
    """
    Loads the mean weights saved in a .json files as a dictionary with key mean_key
    """
    mean_tot = []
    mean_tot = np.asarray(mean_tot, dtype="float64")
    with open(json_path, 'r') as fp:
        annotation = json.load(fp)
        for dof in range(7):
            mean_vector = np.asarray(annotation[dof][mean_key]).round(16).astype("float64")
            mean_tot = np.append(mean_tot, mean_vector)

        return mean_tot     # shape:(56,)
...
def load_batch_weights_mean(dataset_path, ids):
    ...
    """
    return np.asarray([load_weights_mean(os.path.join(dataset_path, id + ".json")) for id in ids],
                      dtype=object)  # My: (950, 56); Alessandra: (252, 8, 1)
...
def load_weights_cov(json_path, cov_key="covariance_vector"):
    """
    Loads the values of the PCA decomposition of the covariance matrix saved in a .json file as a dictionary with key covariance
    """
    cov_tot = []
    cov_tot = np.asarray(cov_tot, dtype="float64")
    with open(json_path, 'r') as fp:
        annotation = json.load(fp)
        for dof in range(7):
            cov_vector = np.asarray(annotation[dof][cov_key]).round(16).astype("float64")
            cov_tot = np.append(cov_tot, cov_vector)

        return cov_tot  # shape:(56,)


def load_batch_weights_cov(dataset_path, ids):
    """
    Loads a batch of the PCA decomposition of the covariance matrix
    """
    return np.asarray([load_weights_cov(os.path.join(dataset_path, id + ".json")) for id in ids],
                      dtype=object)     # My: (950, 63)
...
class Dataset(ABC):
    """
    The base dataset only loads the IDs of the files and the mean and the cov of the ProMPs weights
    """
    ...
    def __init__(self, dataset_dir, rgb_dir):
        ...
        # Load all IDs, images IDs, mean of weights and covariance of weights elements
        ...
        # self.samples["L"] = load_batch_weights_L(self.dataset_dir, self.samples["id"])      # ALESSANDRA
        self.samples["cov_weights"] = load_batch_weights_cov(self.dataset_dir, self.samples["id"])
...
class DatasetOrdinary(Dataset, ABC):
    ...
    def _split_train_test_val(self):
        ...
        for index in idx_all:
            if (self.samples["config"][index] == '5' and self.samples["berries_id"][index] == 'berry3') or \
                    (self.samples["config"][index] == '3' and self.samples["berries_id"][index] == 'berry10') or\
                    (self.samples["config"][index] == '1' and self.samples["berries_id"][index] == 'berry17'):
